...
collection_mongo = database_mongo["20098431"]#
name_collection_qdrant = "20098431"#
...
def insert_data_qdrant():
    try:
        ...
        all_news = collection_mongo.find({"status": "new"})#
        for new in all_news:#
            id = str(new.pop("_id", None))
            embedding = new.pop("embedding", None)
...
with DAG('20098431',
    ...
    t1 = BashOperator(task_id='20098431', bash_command='echo "Midterm exam started"', dag=dag)
    ...
    t2 = PythonOperator(task_id='2009', python_callable=create_collection_qdrant, dag=dag)
    ...
    t3 = PythonOperator(task_id='0098', python_callable=insert_data_mongoDB, dag=dag)
    ...
    t4 = PythonOperator(task_id='0984', python_callable=insert_data_qdrant, dag=dag)
    ...
    t5 = PythonOperator(task_id='9843', python_callable=count_data, dag=dag)
    ...
    t6 = PythonOperator(task_id='8431', python_callable=search_by_vector, dag=dag)
    # task 7: sử dụng BashOperator để in ra "Midterm exam ended" với task_id là 2 số đầu và 2 số cuối của mssv (ví dụ: task_id='1791')
    t7 = BashOperator(task_id='2031', bash_command='echo "Midterm exam ended"', dag=dag)
